...
foreach $i (@cov){
   ...
   system ("cat /storage9/db/Illumina/Platinum/ERP001960/fastq/ERR194146_*.fastq | depth_of_coverage_dilutions.pl $i | tarmo_ref_minus_fastq.pl | sort | uniq > $outfile");
}
...
foreach $i (@cov){
   ...
   system ("gtester -i human_37_25.index  -g /storage7/ctg/uued/tester/human_37.names -3p 10 -f $infile > $outfile");
}
...
foreach $i (@cov){
   ...
   system ("cat $infile | perl tarmo_ref_minus_post_gtester.pl > $outfile");
}
